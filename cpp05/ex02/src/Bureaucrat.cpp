...
//! new function
void Bureaucrat::executeForm(const Form & form) const //можно и так (Form const & form)
{
	try
	{
		form.execute(*this);
	...
	{
		std::cerr <<"\033[31m" << _name << "\033[31m Cannot execute \033[0m" << form.getName() << " because " << exc.what() << std::endl;
		return;
	}
	std::cout << _name << "\033[32m executed \033[0m" << form.getName() << std::endl;
}
